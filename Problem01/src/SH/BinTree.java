...
public class BinTree<K,V> {
	...
	private Node<K,V> root;								// 루트에 대한 참조를 보존, 유지하는 필드
	// Lower Bounded Wildcards --> 지정된 타입과 상위타입만 허용. ex) List<? super Integer>로 정의하면 Integer와 Integer의 상위인 Number, Object 만 가능
	private Comparator<? super K> comparator = null;	// 키 값의 대소 관계를 비교하는 비교자
	
	// 자연 순서에 따라 키 값을 비교, root = null 로 하여 노드가 하나도 없는 이진검색트리를 생성하는 생성자
	BinTree() {
		root = null;
	}
	...
	private int comp(K key1, K key2) {
		// 생성자 BinTree()로 이진검색트리를 생성한 경우 comparator == null 이므로 비교자가 설정되어 있지 않아 key1 을 Comparable<K> 인터페이스형으로 형변환 후 compareTo 메소드로 key2와 비교
	...
	// node를 루토르 하는 서브 트리에 노드<K, V> 를 삽입
	private void addNode(Node<K,V> node, K key, V data) {
		...
		if (cond == 0) {
			...
			if (node.left == null) {
				...
				addNode(node.left, key, data);	// 왼쪽 서브 트리로 재귀
			}
		} else {
			if (node.right == null) {
				node.right = new Node<K,V>(key, data, null, null);
			} else {
				addNode(node.right, key, data);	// 오른쪽 서브 트리로 재귀
			}
		}
	}
	...
	public boolean remove(K key) {
		...
		} else if (p.right == null) {			// p 에는 오른쪽 자식이 없음
			if (p == root) {
				root = p.left;
			} else if (isLeftChild) {
